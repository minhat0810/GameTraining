...
class CircleCollider extends Collider {
    constructor(x, y, radius,speed,imageSrc = null, type ='fruit') {
        super(x, y);
        this.radius = radius;
        this.speed = speed;
        this.isColliding = false; // Trạng thái va chạm
...
class RectCollider extends Collider {
    constructor(x, y, width, height,speed,imageSrc = null) {
        ...
        this.currentFrame = 0;
        this.frameDuration = 100; // Duration for each frame
    ...
    updatePosition(deltaTime){
           const directions = ["ArrowLeft", "ArrowRight"];
            ...
            directions.forEach((key) => {
             ...
             if (!isMoving) {
                this.imgPlayerR.src = "./img/Player/pikachu_tacke.jpg"; // Hình ảnh mặc định
            }
    ...
    updateAnimation() {
        // thay đổi animation dựa trên hướng
...
function spawnRandomCircle() {
    ...
    const circle = new CircleCollider(x, y, radius, speed, imageSrc,'fruit');
    randomCircles.push(circle);
    collisionManager.addCollider(circle);
}
setInterval(spawnRandomCircle, 2000); 
...
function spawnHealth() {
    ...
    const imageSrc = "./img/electrostone.png";
    const circle = new CircleCollider(x, y, radius, speed, imageSrc,'stone');
    randomCircles.push(circle);
    collisionManager.addCollider(circle);
}
setInterval(spawnHealth, 12000); 
...
function gameLoop(timeStamp) {
    ...
    lastTime = timeStamp;
     ...
     if (gameManager.state === 'gameOver') {
        ...
        // const imgPlayerDie = "./img/Player/pikachu_tacke.jpg";
        // const player = new RectCollider(canvas.width/2-50, canvas.height-200, 100, 80,350,imgPlayerDie);
